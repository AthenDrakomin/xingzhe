...
const SideChannel: React.FC = () => {
  return (
    <div className="fixed left-2 sm:left-3 md:left-6 top-1/2 -translate-y-1/2 z-40 flex flex-col gap-3 sm:gap-4 md:gap-6 animate-fade-in-up hidden md:flex" style={{ animationDelay: '500ms' }}>
...
const ChannelItem: React.FC<ChannelItemProps> = ({ href, label, children }) => (
  <a 
    ...
    className="group relative flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-slate-800 bg-slate-950/30 text-slate-500 hover:text-emerald-400 hover:border-emerald-500/50 hover:bg-slate-900 transition-all duration-300 hover:scale-110"
    aria-label={label}
  >
    {children}
